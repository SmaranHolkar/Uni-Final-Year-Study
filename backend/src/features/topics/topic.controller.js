...
Return ONLY valid JSON matching this exact schema (no markdown, no preamble):
{
  ...
  "category": "Related Topic"
    ...
    const raw = await getChatCompletion(prompt, 'qwen/qwen3.6-27b', 0.4, 400, { forceJson: false });
...
- answer must be exactly one letter: 'A', 'B', 'C', or 'D'`;
    ...
    const raw = await getChatCompletion(prompt, 'qwen/qwen3.6-27b', 0.6, 500, { forceJson: false });
