          ...
          if (activeRecallMode) {
            ...
            return (
              <span
                ...
                className={`inline-block px-1.5 py-0.2 mx-0.5 rounded cursor-pointer transition-all ${isRevealed
      ...
      <div className="canvas-node-drag-handle flex flex-wrap items-center justify-between gap-2.5 border-b border-[#1b2636] px-3.5 py-2.5 bg-[#0d1422]">
        ...
        <div className="flex items-center gap-1.5 nodrag">
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-lg text-[10px] font-semibold flex items-center gap-1.5 transition-all shadow-sm ${activeRecallMode
              ...
              {paragraphs.map((p, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-2 rounded-lg text-[11px] transition-all flex flex-col gap-0.5 ${activePage === pageNum
                        ...
                        {visibleItems.length === 0 ? (
                          ...
                          visibleItems.map((item, idx) => {
                            ...
                            return (
                              <div
                                key={item.id || idx}
                                className={`group/item relative rounded-xl p-3 -mx-3 transition-all ${isReadingThis ? 'bg-blue-500/10 ring-2 ring-blue-500/40' : 'hover:bg-slate-500/5'
                                  }`}
                              >
                                {item.heading && (
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 scrollbar-thin">
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-2.5 py-0.5 rounded-full text-[11px] font-medium transition-all capitalize whitespace-nowrap ${data.category === cat
...
export default function Learningplayground() {
  ...
  // PRO Pedagogical & Metacognitive State
  const [isSocraticMode, setIsSocraticMode] = useState(false)
  const activeMetacognitiveContext = location.state?.analysis || location.state?.metacognitiveAnalysis || null
  const activeQuizResults = location.state?.quizResults || null
  const activeWrongQuestions = location.state?.wrongQuestions || null
  const initialPromptFromState = location.state?.initialPrompt || null
  ...
  // Link Study Session (Quiz History) & Metacognitive State
  const [showLinkHistoryModal, setShowLinkHistoryModal] = useState(false)
  const [studyHistoryList, setStudyHistoryList] = useState([])
  const [isLoadingStudyHistory, setIsLoadingStudyHistory] = useState(false)
  const [linkedStudySession, setLinkedStudySession] = useState(null)
  const [linkedMetacognitiveContext, setLinkedMetacognitiveContext] = useState(null)
  const [studyHistorySearch, setStudyHistorySearch] = useState('')

  const effectiveMetacognitiveContext = linkedMetacognitiveContext || activeMetacognitiveContext

  // Fetch past quizzes and study sessions
  const fetchStudyHistory = async () => {
    if (!session?.access_token) return
    setIsLoadingStudyHistory(true)
    try {
      ...
      if (res.ok) {
        ...
        setStudyHistoryList(data.data || [])
      }
    } catch (err) {
      console.error('Error fetching study history for linking:', err)
    } finally {
      setIsLoadingStudyHistory(false)
    }
  }

  // Link a specific study session / quiz into the studio
  const handleLinkStudySession = async (studySession) => {
    setLinkedStudySession(studySession)
    setShowLinkHistoryModal(false)

    // Parse quiz content
    let parsedQuiz = studySession.quiz
    if (typeof parsedQuiz === 'string') {
      try { parsedQuiz = JSON.parse(parsedQuiz) } catch { parsedQuiz = [] }
    }
    ...
    // Set active context & quiz results
    const scoreVal = studySession.score ?? Math.round(((questions.length - wrongQuestions.length) / Math.max(1, questions.length)) * 100)

    // Attempt to fetch full Metacognitive Analysis for this session
    let fetchedMeta = null
    try {
      const metaRes = await fetch(`${API_BASE}/api/metacognitive-analysis/${studySession.id}`, {
        headers: { Authorization: `Bearer ${session.access_token}` },
        credentials: 'include',
      })
      if (metaRes.ok) {
        const metaData = await metaRes.json()
        if (metaData.success && metaData.analysis) {
          fetchedMeta = metaData.analysis
          setLinkedMetacognitiveContext(metaData.analysis)
        }
      }
    } catch (metaErr) {
      console.warn('[LINK STUDY] Could not fetch metacognitive analysis:', metaErr.message)
    }

    // Attempt to resolve and attach the raw document & PDF from bucket
    try {
      const docRes = await fetch(`${API_BASE}/api/documents/content?title=${encodeURIComponent(studySession.title)}`, {
        headers: { Authorization: `Bearer ${session.access_token}` },
        credentials: 'include',
      })
      if (docRes.ok) {
        const docData = await docRes.json()
        if (docData.success && (docData.paragraphs?.length > 0 || docData.fileUrl)) {
          setAttachedDocument({
            id: docData.title,
            title: docData.title,
      ...
      console.warn('[LINK STUDY] Could not auto-resolve document:', err.message)
    }

    // Greet and provide instant tailored AI study direction with metacognitive insights
    const calScore = fetchedMeta?.algorithmicMetrics?.confidenceAnalysis?.calibrationScore ?? fetchedMeta?.calibrationScore
    const pattern = fetchedMeta?.patternSpecificity
    const confMismatch = fetchedMeta?.confidenceMismatch

    let metaDetails = ''
    if (calScore != null) metaDetails += `\n- 🎯 **Confidence Calibration**: **${calScore}%**`
    if (pattern) metaDetails += `\n- 🧠 **Mistake Pattern**: ${pattern}`
    if (confMismatch) metaDetails += `\n- ⚖️ **Calibration Feedback**: ${confMismatch}`

    const introMsg = {
      role: 'assistant',
      content: `🔗 **Linked Study Session: "${studySession.title}"**\n\nI have loaded your session data (Score: **${scoreVal}%**${wrongQuestions.length > 0 ? `, ${wrongQuestions.length} mistake${wrongQuestions.length === 1 ? '' : 's'}` : ''})${metaDetails ? ` and your **Metacognitive Analysis**:\n${metaDetails}` : '.'}\n\nHow would you like to study this topic?\n- **Targeted Practice**: Generate active recall questions on what you missed\n- **Calibrate Blindspots**: Review high-confidence errors and misconceptions\n- **Interactive Tools**: Build flashcards, matching pairs, or diagrams`,
      id: Date.now() + 1,
    }
  ...
  const handleUnlinkStudySession = () => {
    setLinkedStudySession(null)
    setLinkedMetacognitiveContext(null)
  }
  ...
  const handleSendMessage = async (customPrompt = null) => {
    ...
    setGenerationStage(
      isSocraticMode
        ? 'Applying Socratic guided pedagogy...'
        : attachedDocument
          ? `Processing RAG vector context from "${attachedDocument.title}"...`
    ...
    try {
      ...
      if (isSocraticMode) {
        effectivePrompt = `[SOCRATIC TEACHING MODE: Act as a master pedagogical tutor. Guide the student step-by-step through first-principles reasoning and question-driven discovery. If generating an interactive study tool, structure it with step-by-step hints, formative explanations, and metacognitive reflection prompts.]\n\n${textToSend}`
      }
      ...
      const bodyPayload = {
        ...
        metacognitiveAnalysis: effectiveMetacognitiveContext || undefined,
        metacognitiveContext: effectiveMetacognitiveContext || undefined,
  ...
  // Initialize study session if navigated from Metacognitive Analysis, Quiz History, or Grounded Studio
  const hasInitializedFromLocationState = useRef(false)
  useEffect(() => {
    if (!hasInitializedFromLocationState.current && (initialPromptFromState || activeMetacognitiveContext)) {
      hasInitializedFromLocationState.current = true
      if (initialPromptFromState) {
        handleSendMessage(initialPromptFromState)
      } else if (activeMetacognitiveContext) {
        const welcomePrompt = `Review my latest metacognitive analysis and summarize my key confidence calibration strengths and weaknesses.`
        handleSendMessage(welcomePrompt)
      }
    }
  }, [initialPromptFromState, activeMetacognitiveContext])
  ...
  return (
    <main className="main-content h-screen max-h-screen overflow-hidden relative flex flex-col bg-[#090d14] text-slate-100 font-sans" style={{ minHeight: '100vh', height: '100vh', display: 'flex', flexDirection: 'column' }}>

      {/* TOP STUDIO TOOLBAR (Edge-to-edge dedicated header) */}
      <header className="w-full flex-shrink-0 bg-[#0c1017] border-b border-[#223247] px-3.5 py-2 flex items-center justify-between gap-3 z-30 shadow-md">
        {/* Left Toolbar Items */}
        <div className="flex items-center gap-2 flex-wrap">
          {activeCanvasView === 'playground' && (
            <button
              ...
              className={`rounded-lg border px-3 py-1.5 text-xs font-semibold shadow-md transition-all flex items-center gap-2 ${isChatPanelOpen
                  ? 'border-[#282E38] bg-[#1a2130] text-[#cbd5e1] hover:bg-[#222c40] hover:text-white'
                  : 'border-blue-500/50 bg-blue-600 text-white shadow-lg shadow-blue-600/20 hover:bg-blue-500 active:scale-95'
                }`}
            ...
            >
              ...
              {messages.length > 0 && (
                <span className={`rounded-full px-1.5 py-0.2 text-[10px] font-mono ${isChatPanelOpen ? 'bg-[#282E38] text-[#94a3b8]' : 'bg-white/20 text-white'
                  }`}>
                  {messages.length}
                </span>
              )}
          ...
          {/* Header Branding */}
          <div className="hidden xl:flex items-center gap-2.5 bg-[#121824]/90 border border-[#282E38] rounded-lg px-3 py-1.5 shadow-sm backdrop-blur-sm">
            <div className="flex items-center gap-1.5">
              <span className="text-xs font-semibold text-white tracking-tight">HydrusLearn Studio</span>
            </div>
            <span className="text-[#282E38]">|</span>
            <span className="text-[10px] text-slate-400 font-medium truncate max-w-[240px]">
              Interactive Study &amp; Metacognition Studio
            </span>
          </div>

          {/* Socratic Mode Toggle Pill */}
          <button
            onClick={() => setIsSocraticMode((prev) => !prev)}
            className={`rounded-lg border px-2.5 py-1.5 text-xs font-semibold shadow-md transition-all flex items-center gap-1.5 ${isSocraticMode
                ? 'border-indigo-500/60 bg-indigo-600 text-white shadow-indigo-500/20'
                : 'border-slate-700 bg-slate-800/90 text-slate-300 hover:bg-slate-700 hover:text-white'
              }`}
            title={isSocraticMode ? 'Socratic Guided Mode Active' : 'Enable Socratic Guided Teaching Mode'}
          >
            <Brain className={`w-3.5 h-3.5 ${isSocraticMode ? 'text-white animate-pulse' : 'text-indigo-400'}`} />
            <span className="hidden sm:inline">Socratic Mode</span>
            <span className={`text-[10px] px-1 py-0.2 rounded font-mono ${isSocraticMode ? 'bg-white/20 text-white font-bold' : 'bg-slate-700 text-slate-400'}`}>
              {isSocraticMode ? 'ON' : 'OFF'}
            </span>
          </button>

          {/* Metacognition Calibration Quick Action */}
          {(activeMetacognitiveContext || activeQuizResults) && (
            <button
              onClick={() => {
                const weakPrompt = `Review my metacognitive weak spots and misconceptions from my latest analysis, and generate targeted active recall practice to calibrate my confidence.`
                handleSendMessage(weakPrompt)
              }}
              className="rounded-lg border border-purple-500/40 bg-purple-600/20 hover:bg-purple-600/30 text-purple-200 px-2.5 py-1.5 text-xs font-semibold shadow-md transition-all flex items-center gap-1.5"
              title="Generate targeted practice targeting calibration weak spots"
            >
              <Target className="w-3.5 h-3.5 text-purple-400" />
              <span className="hidden sm:inline">Calibrate Weaknesses</span>
            </button>
          )}
        </div>
        ...
        {/* Right Toolbar Items */}
        <div className="flex items-center gap-2 flex-wrap">
          {/* View Switcher: Playground | My Tools | Marketplace */}
          <div className="flex items-center gap-1 bg-slate-800/95 border border-slate-700 rounded-lg p-1 shadow-md">
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-semibold transition-colors ${activeCanvasView === 'playground'
                  ? 'bg-blue-600 text-white shadow-sm'
                  : 'text-slate-300 hover:bg-slate-700 hover:text-white'
                }`}
              title="Interactive Playground Canvas"
            >
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-semibold transition-colors ${activeCanvasView === 'my-tools'
                  ? 'bg-blue-600 text-white shadow-sm'
                  : 'text-slate-300 hover:bg-slate-700 hover:text-white'
                }`}
              title="My Saved Tools Library"
            >
              <Bookmark className="w-3.5 h-3.5 text-amber-400" />
              <span className="hidden sm:inline">My Tools</span>
              {savedTools.length > 0 && (
                <span className="rounded-full bg-slate-700 px-1.5 py-0.2 text-[10px] font-mono text-slate-300">
                  {savedTools.length}
                </span>
              )}
            </button>
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-semibold transition-colors ${activeCanvasView === 'marketplace'
                  ? 'bg-emerald-600 text-white shadow-sm'
                  : 'text-slate-300 hover:bg-slate-700 hover:text-white'
                }`}
              title="Community Tool Marketplace"
            >
          ...
          {/* Chat Sessions History Drawer Button */}
          <button
            onClick={() => setShowHistoryDrawer(true)}
            className="inline-flex items-center gap-1.5 rounded-lg border border-slate-700 bg-slate-800/95 px-3 py-1.5 text-xs font-semibold text-slate-200 shadow-md transition-colors hover:bg-slate-700 hover:text-white"
            title="Open previous chat & studio conversation history"
          >
            <Clock className="w-3.5 h-3.5 text-blue-400" />
            <span className="hidden md:inline">Chat History</span>
            {chatHistory.length > 0 && (
              <span className="rounded-full bg-blue-500/20 px-1.5 py-0.2 text-[10px] font-mono text-blue-300 border border-blue-500/30">
                {chatHistory.length}
              </span>
            )}
          </button>

          {/* Link Study Session / Quiz History Button */}
          <button
            onClick={() => {
              setShowLinkHistoryModal(true)
              fetchStudyHistory()
            }}
            className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-semibold shadow-md transition-all ${linkedStudySession
                ? 'border-indigo-500 bg-indigo-600/30 text-indigo-200 shadow-indigo-500/20'
                : 'border-slate-700 bg-slate-800/95 text-slate-200 hover:bg-slate-700 hover:text-white'
              }`}
            title="Link a previous study session or quiz to get focused help"
          >
          ...
          {/* Link to Interactive Study Session (connects Metacognition) */}
          <Link
            to="/Learningpage"
            state={{
              metacognitiveContext: activeMetacognitiveContext,
              quizResults: activeQuizResults,
              analysis: activeMetacognitiveContext,
            }}
            className="inline-flex items-center gap-1.5 rounded-lg border border-indigo-500/40 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white px-3 py-1.5 text-xs font-semibold shadow-md transition-all"
            title="Launch interactive Study Session (auto-calibrates Metacognition)"
          >
          ...
          {activeCanvasView === 'playground' && (
            <>
              ...
              <div className="hidden 2xl:flex items-center gap-1 bg-slate-800/95 border border-slate-700 rounded-lg p-1 shadow-md">
                <button
                  onClick={() => handleAddStickyNote(380, 140)}
                  className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-semibold text-slate-200 hover:bg-slate-700 hover:text-white transition-colors"
                ...
                {attachedDocument && (
                  <>
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-semibold text-blue-300 bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/30 transition-colors shadow-sm"
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-semibold text-slate-300 hover:bg-slate-700 hover:text-white transition-colors"
              ...
              <button
                ...
                className="hidden sm:inline-flex items-center gap-1.5 rounded-lg border border-slate-700 bg-slate-800/95 px-3 py-1.5 text-xs font-semibold text-slate-200 shadow-md transition-colors hover:bg-slate-700 hover:text-white"
              ...
              <button
                ...
                className={`rounded-lg border px-3 py-1.5 text-xs font-semibold shadow-md transition-colors ${rightPanelOpen || generatedTool ? 'border-blue-500 bg-blue-600 text-white' : 'border-slate-700 bg-slate-800/95 text-slate-300 hover:bg-slate-700 hover:text-white'}`}
      ...
      {/* ACTIVE LINKED STUDY SESSION BANNER */}
      {linkedStudySession && (
        <div className="w-full bg-[#111827] border-b border-indigo-500/30 px-3.5 py-1.5 flex items-center justify-between gap-3 text-xs shadow-inner z-20 animate-fade-in flex-shrink-0">
          <div className="flex items-center gap-2.5 min-w-0">
            <span className="flex h-2 w-2 rounded-full bg-emerald-400 animate-pulse flex-shrink-0" />
            <div className="flex items-center gap-2 truncate">
              <span className="font-bold text-white truncate text-xs">
                🔗 Linked Study: <span className="text-indigo-300">{linkedStudySession.title}</span>
              </span>
              {linkedStudySession.score != null && (
                <span className="px-2 py-0.5 rounded bg-indigo-500/20 text-indigo-300 font-mono text-[10px] font-bold border border-indigo-500/30 flex-shrink-0">
                  {linkedStudySession.score}%
                </span>
              )}
              {effectiveMetacognitiveContext?.algorithmicMetrics?.confidenceAnalysis?.calibrationScore != null && (
                <span className="hidden sm:inline-flex px-2 py-0.5 rounded bg-purple-500/20 text-purple-300 font-mono text-[10px] font-semibold border border-purple-500/30 flex-shrink-0 items-center gap-1" title="Confidence Calibration Score from Mind's Mirror">
                  <Brain className="w-3 h-3 text-purple-400" />
                  Calib: {effectiveMetacognitiveContext.algorithmicMetrics.confidenceAnalysis.calibrationScore}%
                </span>
              )}
              {effectiveMetacognitiveContext?.patternSpecificity && (
                <span className="hidden lg:inline-flex text-[10px] text-slate-400 truncate max-w-xs" title={effectiveMetacognitiveContext.patternSpecificity}>
                  • {effectiveMetacognitiveContext.patternSpecificity}
                </span>
              )}
            </div>
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <button
              onClick={() => handleSendMessage(`Review my metacognitive blindspots and mistakes in "${linkedStudySession.title}", and generate 5 targeted active recall questions to calibrate my understanding.`)}
              className="px-2.5 py-1 rounded-md bg-indigo-600 hover:bg-indigo-500 text-white text-[11px] font-semibold shadow-sm transition-colors flex items-center gap-1"
              title="Generate targeted practice targeting calibration weak spots"
            >
            ...
            {attachedDocument && (
              <button
                onClick={() => handleAddPdfReaderNode(attachedDocument.title, attachedDocument.fileUrl)}
                className="px-2.5 py-1 rounded-md bg-slate-800 hover:bg-slate-700 text-slate-200 text-[11px] font-semibold border border-slate-700 transition-colors flex items-center gap-1"
                title="Open PDF Reader for this linked session"
              >
            ...
            <button
              onClick={handleUnlinkStudySession}
              className="p-1 rounded-md text-slate-400 hover:text-white hover:bg-slate-800 text-xs transition-colors"
              title="Unlink study session"
            >
              <X className="w-3.5 h-3.5" />
            </button>
          </div>
      ...
      {/* FULL-WIDTH DUAL PANE WORKSPACE (CHAT + CANVAS) */}
      <div className="flex-1 min-h-0 flex flex-row relative overflow-hidden">
        ...
        {activeCanvasView === 'playground' && (
          <div
            className={`relative z-20 flex flex-col bg-[#10151f]/95 backdrop-blur-xl transition-[transform,opacity] ${isChatPanelResizing ? 'transition-none select-none' : 'duration-300'
              } flex-shrink-0 overflow-hidden ${isChatPanelOpen
                ...
                <div className="mt-1.5 flex items-center justify-between gap-1.5 border-t border-[#1d293d] pt-1.5">
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      className={`inline-flex h-7 items-center justify-center rounded-lg px-2 text-xs font-medium transition-colors ${isChatListening
                    ...
                    <button
                      ...
                      className={`inline-flex h-7 items-center justify-center rounded-lg px-2 text-xs font-medium transition-colors ${isSocraticMode
                          ? 'bg-indigo-600/30 text-indigo-300 border border-indigo-500/50 shadow-sm'
                          : 'text-[#8493a8] hover:bg-[#1f2e45] hover:text-white'
                        }`}
                      title={isSocraticMode ? 'Socratic Guided Mode Active' : 'Toggle Socratic Teaching Mode'}
                    >
                      <Brain className="w-3.5 h-3.5 mr-1" />
                      <span className="text-[10px] font-medium">{isSocraticMode ? 'Socratic: ON' : 'Socratic'}</span>
                    </button>
                  </div>
          ...
          >
            ...
            {isDraggingOver && (
              <div className="absolute inset-4 z-40 flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-blue-400/80 bg-[#0c1017]/90 backdrop-blur-md shadow-2xl pointer-events-none">
                <div className="w-14 h-14 rounded-2xl bg-blue-500/20 border border-blue-400/40 flex items-center justify-center text-blue-300 mb-3">
                                  ...
                                  return (
                                    <div key={letter} className="flex items-center gap-1.5">
                                      <button
                                        ...
                                        className={`w-7 h-7 rounded-[4px] text-xs font-bold font-mono transition-all flex items-center justify-center flex-shrink-0 ${isCorrect
                                      ...
                                      <input
                                        ...
                                        className={`flex-1 bg-[#1A1E24] border rounded-[6px] px-2.5 py-1.5 text-xs text-white placeholder-[#6E7580] focus:outline-none ${isCorrect ? 'border-emerald-500/60 bg-emerald-950/10' : 'border-[#282E38] focus:border-[#5A7D99]'
                                          }`}
                                      />
                                    </div>
                                  )
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex items-center gap-2">
                                <button
                                  ...
                                  className={`px-3 py-1 rounded-[4px] text-xs font-bold transition-all ${item.isTrue !== false
      ...
      {/* CHAT CONVERSATIONS HISTORY DRAWER */}
      {showHistoryDrawer && (
        <div
        ...
        >
          <div
            className="w-full max-w-sm bg-[#10151f] border-l border-slate-800 h-full p-4 flex flex-col shadow-2xl"
            ...
            <div className="flex-1 overflow-y-auto py-3 space-y-1.5 scrollbar-thin">
              {chatHistory.length === 0 ? (
                <div className="text-center py-12 text-slate-500 text-xs">
                  No previous chats saved yet. Start a new chat to begin!
                </div>
              ) : (
                chatHistory.map((chat) => (
                  <div
                    ...
                    className={`p-3 rounded-lg border text-xs cursor-pointer transition-all flex items-center justify-between group ${activeChatId === chat.id
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="font-semibold truncate">{chat.title || 'Previous Chat'}</p>
                        {chat.updatedAt && (
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 p-1 text-slate-500 hover:text-red-400 hover:bg-red-500/10 rounded transition-all ml-2"
      ...
      {/* LINK STUDY SESSION / QUIZ HISTORY MODAL */}
      {showLinkHistoryModal && (
        <div
          ...
          onClick={() => setShowLinkHistoryModal(false)}
        >
          <div
            className="w-full max-w-xl bg-[#10151f] border border-slate-800 rounded-2xl p-5 shadow-2xl space-y-4 max-h-[85vh] flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm font-bold text-white">Link a Study Session to Studio</h3>
                  <p className="text-[11px] text-slate-400">
                    Connect a past quiz to get targeted AI help, weak spot practice, and grounded PDF citations
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search previous study sessions by topic or document title..."
                className="w-full pl-9 pr-3 py-2 bg-slate-900 border border-slate-700/80 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-colors"
              />
            </div>

            <div className="flex-1 overflow-y-auto space-y-2 py-1 scrollbar-thin">
              {isLoadingStudyHistory ? (
                <div className="py-12 text-center text-slate-400 text-xs flex flex-col items-center gap-2">
                  <div className="w-6 h-6 border-2 border-indigo-400 border-t-transparent rounded-full animate-spin" />
                  <span>Loading study history...</span>
                </div>
              ) : studyHistoryList.length === 0 ? (
                <div className="py-12 text-center text-slate-500 text-xs">
                  No past study sessions found. Start a study session to build your history!
                </div>
              ) : (
                studyHistoryList
                  .filter((item) => (item.title || 'Study Session').toLowerCase().includes(studyHistorySearch.toLowerCase()))
                  .map((item) => {
                    let parsed = item.quiz
                    ...
                    const qCount = Array.isArray(parsed) ? parsed.length : (parsed?.questions?.length || 0)
                    const score = item.score ?? null
                    const isLinked = linkedStudySession?.id === item.id

                    return (
                      <div
                        key={item.id}
                        className={`p-3.5 rounded-xl border transition-all flex items-center justify-between gap-3 group ${isLinked
                            ? 'border-indigo-500/80 bg-indigo-950/30'
                            : 'border-slate-800 bg-slate-900/70 hover:border-indigo-500/40 hover:bg-slate-900'
                          }`}
                      >
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <h4 className="font-semibold text-xs text-white truncate">{item.title || 'Study Session'}</h4>
                            {score != null && (
                              <span
                                className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold ${score >= 80
                        ...
                        <button
                          onClick={() => handleLinkStudySession(item)}
                          className={`px-3 py-1.5 rounded-lg text-xs font-semibold shadow-sm transition-all flex items-center gap-1.5 flex-shrink-0 ${isLinked
                              ? 'bg-emerald-600 hover:bg-emerald-500 text-white'
                              : 'bg-indigo-600 hover:bg-indigo-500 text-white group-hover:scale-105'
                            }`}
                        >
                          <GitFork className="w-3.5 h-3.5" />
                          <span>{isLinked ? 'Re-link' : 'Link to Studio'}</span>
                        </button>
                      </div>
                    )
                  })
            ...
            <div className="pt-3 border-t border-slate-800 flex items-center justify-between text-xs">
              ...
              >
                <span>View full Quiz History &amp; Analytics</span>
