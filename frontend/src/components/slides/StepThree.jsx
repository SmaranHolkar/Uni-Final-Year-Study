...
//       setLoading(true);
//       try {
//         const res = await axios.post("/api/generate-similar-topic", { topic: selectedNode.data.label });
//         const newNode = res.data.node; // { id, label, ... }
//         setNodes((nds) => [...nds, newNode]);
//         setEdges((eds) => [...eds, { id: `e-${Date.now()}`, source: selectedNode.id, target: newNode.id }]);
//       } catch (err) {
//         alert("Failed to add similar topic");
//       }
//       setLoading(false);
//     };
...
//       setLoading(true);
//       try {
//         const res = await axios.post("/api/generate-mcq", { topic: selectedNode.data.label });
//         setMcq(res.data.mcq); // { question, choices, answer }
//       } catch (err) {
...
//   return (
//     <div className="h-[900px] flex flex-col bg-slate-50 rounded-xl overflow-hidden border border-slate-200 shadow-xl relative">
...
//                   onClick={() => {
//                     const link = selectedNode.data.sourceLink;
//                     if (link && link.startsWith('http')) {
//                       window.open(link, "_blank");
//                     } else {
//                       const query = encodeURIComponent(selectedNode.data.label || "study topic");
  ...
  return (
    <div className="h-[900px] relative bg-slate-50 rounded-xl overflow-hidden">
      ...
      <div className="p-4 bg-white border-b flex items-center justify-between">
        ...
        <h1 className="font-bold">Your Review Map</h1>
      </div>
