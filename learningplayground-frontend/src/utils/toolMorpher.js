...
export function morphToolToHtml(targetFormat, title, description, rawItems) {
  ...
  const baseCss = `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .badge { display: inline-flex; align-items: center; padding: 0.2rem 0.6rem; border-radius: 9999px; font-size: 0.75rem; font-weight: 700; background: var(--muted); border: 1px solid var(--border); color: var(--primary); }
    .btn { display: inline-flex; align-items: center; justify-content: center; gap: 0.4rem; padding: 0.55rem 1.15rem; font-size: 0.825rem; font-weight: 600; border-radius: 0.5rem; cursor: pointer; transition: all 0.15s; border: none; }
    ...
    .card { background: var(--card); border: 1px solid var(--border); border-radius: 0.75rem; padding: 1.25rem; box-shadow: 0 4px 20px rgba(0,0,0,0.25); }
    ...
    return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><style>${baseCss}
      ...
      <div id="app">
        <header id="app-header">
          <span class="badge">🗂️ 3D Spaced Flashcards</span>
          ...
          <div style="display:flex;gap:0.5rem;align-items:center;justify-content:center;margin-top:0.75rem;">
            ...
            <span class="badge" style="background:#131519;color:#8E8E93;">⌨️ Space: Flip | ← →: Prev/Next</span>
          </div>
        </header>
        <main id="app-main" style="width:100%;display:flex;flex-direction:column;align-items:center;">
          ...
          <div style="display:flex;gap:0.5rem;align-items:center;justify-content:center;margin-top:0.5rem;">
            ...
            <button class="btn btn-secondary" id="fc-shuffle" title="Shuffle Cards">🔀</button>
          </div>
    ...
    return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><style>${baseCss}
      ...
      <div id="app">
        <header id="app-header">
          ...
          <div style="display:flex;gap:0.75rem;justify-content:center;margin-top:0.5rem;">
            ...
            <span class="badge" id="timer-badge" style="background:#ef4444;color:white;">⏱️ 45s</span>
      ...
      <script>
        ...
        function renderQ() {
          ...
          exp.textContent = '💡 ' + (it.explanation || it.back || 'Correct concept verified.');
  ...
  // 4. CLOZE BLURTER
  if (format.includes('cloze') || format.includes('blurting') || format.includes('fill')) {
    return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><style>${baseCss}
  ...
  if (format.includes('memory') || format.includes('palace') || format.includes('loci') || format.includes('mnemonic')) {
    const LOCI_ROOMS = [
      { name: '🏛️ Grand Foyer & Entrance', icon: '🏛️', color: '#6366f1' },
      ...
      <script>
          ...
          if (testMode) {
            ...
            box.innerHTML =
              ...
              '<div class="mnemonic-scene">' +
                '<strong style="color:#38bdf8;">👁️ Vivid Mnemonic Visualization:</strong><br>' +
        ...
        window.checkTestAnswer = function() {
          ...
          if (inp && (ans.includes(inp) || inp.includes(ans))) {
            fb.innerHTML = '<strong style="color:#10b981;">✅ Spot on!</strong> You accurately recalled: <em>' + (it.front || it.concept) + '</em><br><br>' + (it.back || it.explanation);
          } else {
            fb.innerHTML = '<strong style="color:#f59e0b;">💡 Answer:</strong> <em>' + (it.front || it.concept) + '</em><br><br>' + (it.back || it.explanation);
          }
        };
        ...
        document.getElementById('mp-test-toggle').onclick = () => {
          testMode = !testMode;
          document.getElementById('mp-test-toggle').textContent = testMode ? '👁️ Palace Walk' : '🧩 Active Recall Walk';
          render();
        };
  ...
  // 6. FEYNMAN ACTIVE RECALL GRADER
  if (format.includes('feynman') || format.includes('grader')) {
    return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><style>${baseCss}
      ...
      <div id="app">
        <header id="app-header">
          <span class="badge" style="background:#5A7D99;color:white;">🎙️ Feynman Active Recall Grader</span>
          <h1 style="margin-top:0.35rem;">${title}</h1>
        ...
        <main id="app-main" style="width:100%;display:flex;flex-direction:column;align-items:center;">
          <div class="feynman-box">
            ...
            <div style="display:flex;gap:0.5rem;margin-top:0.75rem;">
              <button class="btn btn-primary w-full" id="f-grade-btn">⚡ Grade My Explanation</button>
      ...
      <script>
        ...
        document.getElementById('f-grade-btn').onclick = () => {
          ...
          keyWords.forEach(kw => {
            ...
            row.innerHTML = (match ? '<span style="color:#10b981;">✅</span> ' : '<span style="color:#f59e0b;">⚠️</span> ') + kw;
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><style>${baseCss}
    ...
    <script>
      ...
      DATA.forEach((item, i) => {
        ...
        c.className = 'card';
        c.innerHTML = '<h3 style="font-weight:700;font-size:1rem;color:var(--primary);margin-bottom:0.35rem;">' + (item.front || item.concept || 'Topic ' + (i + 1)) + '</h3><p class="muted" style="font-size:0.875rem;line-height:1.6;">' + (item.back || item.detail || item.explanation || '') + '</p>';
      ...
      function renderCard() {
        ...
        if (cFlipped) {
          b.textContent = 'Back'; txt.textContent = it.back || it.answer || '';
        } else {
          b.textContent = 'Front (Click to Flip)'; txt.textContent = it.front || it.concept || it.question || '';
        }
      }

      document.getElementById('card-elem').onclick = () => { cFlipped = !cFlipped; renderCard(); };
      document.getElementById('c-flip').onclick = () => { cFlipped = !cFlipped; renderCard(); };
      document.getElementById('c-prev').onclick = () => { cIdx = (cIdx - 1 + DATA.length) % DATA.length; cFlipped = false; renderCard(); };
      document.getElementById('c-next').onclick = () => { cIdx = (cIdx + 1) % DATA.length; cFlipped = false; renderCard(); };
